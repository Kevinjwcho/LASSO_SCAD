...
quantile_hbic <- function(IterN, n, p, mu, vmatirx, tmpB, cfe){
  ...
  for (iter in 1:IterN)
  {
    ...
    BICh5_i<-c() # new_C(n, 1, log(p), denz)
    ...
    for (i in 1:L){
      ...
      BICh5_i<-c(BICh5_i,2*log(sum(quant_loss((Y - X %*% beta_si[-1]), tau))) + 2*new_C(n, gam=log(p), p, denzero(beta_si[-1]))+log(n)*denzero(beta_si[-1])/n)
    }
...
for(j in 1:length(C_list)){
  ...
  for(i in 1:length(p_list)){
    ...
    w <- paste0("quantile_result_ver2_", "p", p, "_C", C_beta)
    assign(w, result)
...
# quantile_result_ver2_p100_C1
# quantile_result_ver2_p200_C1
# quantile_result_ver2_p400_C1
# 
# quantile_result_ver2_p100_C3
# quantile_result_ver2_p200_C3
# quantile_result_ver2_p400_C3
