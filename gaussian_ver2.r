...
gaussian_hbic <- function(IterN, n, p, mu, vmatirx, tmpB, cfe){
  # selection result
  ...
  NC_log <- numeric(length(which(tmpB!=0)))
  NC_newC_gam_0 <- numeric(length(which(tmpB!=0)))
  NC_newC_gam_0.5 <- numeric(length(which(tmpB!=0)))
  NC_newC_gam_1 <- numeric(length(which(tmpB!=0)))
  
  for (iter in 1:IterN)
  {
    if(iter%%10 == 0){cat("Iteration is now ", iter, " of ", IterN, "\n")}
    X <-mvrnorm(n,mu,vmatrix)
    e <- rnorm(n,0,1) # normal error
    ...
    for (i in 1:length(lam_grid)){
      ...
      temp[,ind]<-X[,ind]
      fit1=glmnet(temp,Y,intercept=FALSE)
      #print(fit1)
    ...
    for (i in 1:L){
      beta_si<-beta_sc[i,]    
      # BICh1_i<-c(BICh1_i,log(sum((Y - X %*% beta_si[-1])^2)/n) + (log(n)+log(p))*log(n)*denzero(beta_si[-1])/n)
      BICh1_i<-c(BICh1_i,log(sum((Y - X %*% beta_si[-1])^2)) + log(p)*log(n)*denzero(beta_si[-1])/n)
      BICh2_i<-c(BICh2_i,log(sum((Y - X %*% beta_si[-1])^2)) + 2*new_C(n, gam=0, p, denzero(beta_si[-1]))+log(n)*denzero(beta_si[-1])/n)
      BICh3_i<-c(BICh3_i,log(sum((Y - X %*% beta_si[-1])^2)) + 2*new_C(n, gam=0.5, p, denzero(beta_si[-1]))+log(n)*denzero(beta_si[-1])/n)
      BICh4_i<-c(BICh4_i,log(sum((Y - X %*% beta_si[-1])^2)) + 2*new_C(n, gam=1, p, denzero(beta_si[-1]))+log(n)*denzero(beta_si[-1])/n)
    }
    ...
    NC_newC_gam_0 <- NC_newC_gam_0 + temp2[tindex]
    ...
    NC_newC_gam_0.5 <- NC_newC_gam_0.5 + temp3[tindex]
    ...
    NC_newC_gam_1 <- NC_newC_gam_1 + temp4[tindex]
  ...
  # 
  NC <- rbind(NC_log, NC_newC_gam_0, NC_newC_gam_0.5, NC_newC_gam_1)/IterN
  # 
  ...
  result <- as.data.frame(NC)
  colnames(result) <- apply(as.data.frame(tindex), 1, function(x) paste0("X_", x))
...
for(j in 1:length(C_list)){
  ...
  for(i in 1:length(p_list)){
    ...
    result <- gaussian_hbic(IterN, n, p, mu, vmatirx, tmpB, cfe)
    w <- paste0("gaussian_result_ver2_", "p", p, "_C", C_beta)
    assign(w, result)
    write.csv(get(w), paste0("C:/Users/Cho/Google 드라이브/Paper working/이은령교수님 cowork/LASSO_SCAD/results/results_0903/",w,".csv"))
    
  }
}
Sys.time()-ptm

gaussian_result_ver2_p100_C1
gaussian_result_ver2_p200_C1
gaussian_result_ver2_p400_C1

gaussian_result_ver2_p100_C3
gaussian_result_ver2_p200_C3
gaussian_result_ver2_p400_C3
